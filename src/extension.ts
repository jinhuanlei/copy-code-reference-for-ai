...
function resolveRemoteUrl(
  ...
  if (!parsed) {
    return {
      kind: "error",
      message: `Unsupported remote "${info.remoteUrl}".`,
    };
  }
  ...
  const ref =
    (useCommit ? info.commit : info.branch) ?? info.commit ?? info.branch;
  if (!ref) {
  ...
  return {
    kind: "ok",
    url: buildRemoteUrl(parsed, relativePath, startLine, endLine, ref),
  };
}
...
async function copyFileReference(uri?: vscode.Uri): Promise<void> {
  ...
  const path = mode === "absolute" ? fsPath : hasRelative ? asRel : fsPath;
...
async function copyAs(): Promise<void> {
  ...
  items.push(
    remoteResult.kind === "ok"
      ? {
          label: "$(link) Remote Permalink",
          description: remoteResult.url,
          value: remoteResult.url,
          isRemote: true,
        }
      : {
          label: "$(link) Remote Permalink",
          description: `(${remoteResult.message})`,
          value: null,
          isRemote: true,
        },
  );
...
async function copyRemoteReference(): Promise<void> {
  ...
  if (typeof result === "string") {
    vscode.window.showWarningMessage(
      result.replace("Copy Code Reference", "Copy Remote Reference"),
    );
    return;
  }
  ...
  if (remoteResult.kind === "error") {
    vscode.window.showWarningMessage(
      `Copy Remote Reference: ${remoteResult.message}`,
    );
    return;
  }
