...
suite("normalizeLineRange", () => {
  test("caret (empty selection) returns 1-based single line", () => {
    assert.deepStrictEqual(normalizeLineRange(9, 9, 0, 0), {
      startLine: 10,
      endLine: 10,
    });
  });
  ...
  test("single-line partial selection", () => {
    assert.deepStrictEqual(normalizeLineRange(9, 9, 4, 12), {
      startLine: 10,
      endLine: 10,
    });
  });
  ...
  test("multi-line selection", () => {
    assert.deepStrictEqual(normalizeLineRange(9, 19, 0, 5), {
      startLine: 10,
      endLine: 20,
    });
  });
  ...
  test("trailing-newline selection (end at column 0) trims last line", () => {
    assert.deepStrictEqual(normalizeLineRange(9, 20, 0, 0), {
      startLine: 10,
      endLine: 20,
    });
  });
