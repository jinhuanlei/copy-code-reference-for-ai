...
suite('resolveFormatConfig', () => {
    ...
    test('every FORMAT_PROFILES entry resolves correctly', () => {
        for (const profile of FORMAT_PROFILES) {
            ...
            assert.strictEqual(result.pathLineSeparator, profile.pathLineSeparator, `${profile.id} pathLineSeparator`);
            assert.strictEqual(result.lineRangeSeparator, profile.lineRangeSeparator, `${profile.id} lineRangeSeparator`);
        }
    });
});
