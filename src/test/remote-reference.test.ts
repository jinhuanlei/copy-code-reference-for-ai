...
suite("parseRemoteUrl", () => {
  ...
  suite("GitLab", () => {
    test("HTTPS with .git suffix", () => {
      assert.deepStrictEqual(
        parseRemoteUrl("https://gitlab.com/owner/repo.git"),
        {
          host: "gitlab",
          owner: "owner",
          repo: "repo",
  ...
  suite("Bitbucket", () => {
    test("HTTPS with .git suffix", () => {
      assert.deepStrictEqual(
        parseRemoteUrl("https://bitbucket.org/owner/repo.git"),
        {
          host: "bitbucket",
          owner: "owner",
          repo: "repo",
    ...
    test("SSH colon form", () => {
      assert.deepStrictEqual(
        parseRemoteUrl("git@bitbucket.org:owner/repo.git"),
        {
          host: "bitbucket",
          owner: "owner",
          repo: "repo",
  ...
  test("unknown host returns null", () => {
    assert.strictEqual(
      parseRemoteUrl("https://selfhosted.example.com/owner/repo.git"),
      null,
    );
  });
...
suite("buildRemoteUrl", () => {
  ...
  const gitlab: ParsedRemote = {
    host: "gitlab",
    owner: "alice",
    repo: "myrepo",
  };
  const bitbucket: ParsedRemote = {
    host: "bitbucket",
    owner: "alice",
    repo: "myrepo",
  };
