...
- **Copy Code Reference as...** appears in the editor's right-click menu under the Copy/Paste section.
...
The **Copy Code Reference as...** command opens a menu showing every available format with a live preview of the output for your current selection:
...
Set `copyCodeRefForAi.format` to one of the built-in profiles to switch formats without touching the individual separator settings:
...
Existing users with custom separator settings see no change — the default is `"custom"`.
